...
with open('llrpdef.py', 'w', encoding='utf8') as fp:
	fp.write( '#-----------------------------------------------------------\n' )
	fp.write( '# DO NOT EDIT!\n' )
	fp.write( '# MACHINE GENERATED by ParseDef.py\n' )
	fp.write( '# XML sources:\n' )
	fp.write(f'#    "{llrpDefXml}"\n' )
	fp.write(f'#    "{llrpCustomDefXml}"\n' )
	fp.write( '# see https://github.com/esitarski/pyllrp for details\n' )
	fp.write( '#-----------------------------------------------------------\n' )
	for n in ('vendors', 'enums', 'parameters', 'messages', 'choiceDefinitions'):
		fp.write( f'{n}=' )
		pprint.pprint( globals()[n], fp, compact=True )
		fp.write( '\n' )
